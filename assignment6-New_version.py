...
class NaiveBayes:
    ...
    def __init__(self, filename: str):
        ...
        #
        #
        self.action_list = ['R','L','F','B']
        self.dist_list = ['C','N','M','F','L']
        self.angle_list = ['C','R','Z','B','A','L']

        self.likelihood_Table = dict()
        
        
        # liklihood_Table[Sensor][action_list][Label]
        for _ in data.iloc[:,:-4]:
            print('Header : ',_)
            self.likelihood_Table[_] = dict
            for action in self.action_list:
                # self.likelihood_Table[_][action] = dict()
                print(f"Action: {action}")
                temp = data[data['action'] == action]
                # print(temp[_].value_counts())
                self.likelihood_Table[_].update( {action : temp[_].value_counts().to_dict()} )
        print("Likelihood Table")
        for _ in data.iloc[:,:-4]:
            print(self.likelihood_Table[_]['R'])
